...
Features  <- subset(Features, Features$X0 != 'vitals_missing')
...
must_have_names <- c('Age', 'Female','Union')
...
for (cvi in 1:(Ncv+1)) {
    ...
    if (feautre_type!='baseline') {
        ...
        coef_ <- as.numeric(coef(coxph_fit, s=coxph_fit$lambda[coxph_fit$index['1se',]]))
        s=coxph_fit$lambda[coxph_fit$index['1se',]]
