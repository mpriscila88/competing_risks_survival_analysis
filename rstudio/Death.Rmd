...
df_AJ <- df
df_AJ$event <- factor(df_AJ$event, 0:1, labels=c("censor", "event1"))
AJ_fit <- survfit(Surv(duration, event) ~ 1, data=df_AJ, id=id)
writeMat(file.path(result_path, sprintf('AJ_output_%s_%s.mat', outcome, model_type)), time=AJ_fit$time, val=AJ_fit$pstate, lower=AJ_fit$lower, upper=AJ_fit$upper, states=AJ_fit$states)
...
Features  <- subset(Features, Features$X0 != 'A1C')
Features  <- subset(Features, Features$X0 != 'AST')
Features  <- subset(Features, Features$X0 != 'ALT')
Features  <- subset(Features, Features$X0 != 'LDL')
Features  <- subset(Features, Features$X0 != 'Albumin')
...
Features  <- subset(Features, Features$X0 != 'SystolicBP')
Features  <- subset(Features, Features$X0 != 'DiastolicBP')
Features  <- subset(Features, Features$X0 != 'Union')

Features  <- subset(Features, Features$X0 != 'BMI')

Features  <- subset(Features, Features$X0 != 'AlcoholDrinksPerWeek')
Features  <- subset(Features, Features$X0 != 'Smoker')

Features  <- subset(Features, Features$X0 != 'RespirationRate')
...
indexes <- unique(df$Indexes)
outer_cv_index_foldid <- as.numeric(fold(as.data.frame(1:length(indexes)), Ncv)$.folds)
names(outer_cv_index_foldid) <- indexes
outer_cv_foldid <- c()
for (i in 1:nrow(df))
    outer_cv_foldid <- c(outer_cv_foldid, outer_cv_index_foldid[[toString(df$Indexes[i])]])
...
for (cvi in 1:(Ncv+1)) {
    ...
    ytr <- Surv(dftr[,'duration'], dftr[,'event'])
    ...
    #dftr$event <- factor(dftr$event, 0:1, labels=c("censor", "event1"))
    ...
    # save predicted z
    zptr1 <- drop(as.matrix(dftr[,xnames2])%*%coxph_coef[1:nrow(coxph_coef),'coef'])
    ...
    writeMat(file.path(result_path, sprintf('results_%s_%s_cv%d.mat', outcome, model_type, cvi)), cindex_tr=coxph_fit_summary$concordance, transition=coxph_fit$transition, Xnames=c(xnames,'Age', 'Sex'), Xnames2=xnames2, coef=coxph_coef, zptr1=zptr1, best_l1_ratio=alphas, Xtr=Xtr) 
}


dfte <- read.csv(file.path(base_folder, 'te_curves.csv'))

dfte2 <- dfte[, (names(dfte) %in% xnames2)] # vars only in list vars

dfte$id <- 1:nrow(dfte)
Xte <- as.matrix(dfte[,xnames2])

zpte1 <- drop(as.matrix(Xte)%*%coxph_coef[1:nrow(coxph_coef),'coef'])
names(zpte1) <- NULL

yp <-survfit(coxph_fit, dfte2)

writeMat(file.path(result_path, sprintf('coxph_scores_%s.mat', outcome)),
         survtime=yp$time, survprob=yp$surv)
...
yp <-survfit(coxph_fit, dfte2[idx,])

writeMat(file.path(result_path, sprintf('survival_curves_te_2%s.mat', outcome)),
         survtime=yp$time, survprob=yp$surv)
...
for (i in 1:3) {
  ...
  df_AJ$event <- factor(df_AJ$event, 0:1, labels=c("censor", "event1"))
  AJ_fit <- survfit(Surv(duration, event) ~ 1, data=df_AJ, id=id)
  writeMat(file.path(result_path, sprintf('AJ_output_te_2%s_%s.mat', outcome, this_data)), time=AJ_fit$time, val=AJ_fit$pstate, lower=AJ_fit$lower, upper=AJ_fit$upper, states=AJ_fit$states)
}
...
dfte <- read.csv(file.path(base_folder, 'tr_curves.csv'))
...
# redefine y
xte$event <- factor(xte$event, 0:1, labels=c("censor", "event1"))

# Fit survival data using the Kaplan-Meier method
y <- Surv(time = xte$duration, event = xte$event) 

# reproduce coxph.fit$concordance
zpte1 <- drop(as.matrix(xte[,xnames2])%*%coxph_coef[1:nrow(coxph_coef),'coef'])

names(zpte1) <- NULL

y <- aeqSurv(y)
y <-Surv(c(y[,1]),c(as.integer(y[,2]==1)))

res <- concordancefit(y, zpte1, reverse=TRUE, timefix=FALSE)
...
for (iter in 0:999) {
  ...
  # reproduce coxph.fit$concordance
  zpte <- drop(as.matrix(xte[,xnames2])%*%coxph_coef[1:nrow(coxph_coef),'coef'])
  names(zpte) <- NULL
  ...
  zmean_m_std <- zmean - zstd
  zmean_p_std <- zmean + zstd
  zperc <- quantile(zpte, c(0,0.005,0.01,0.025,0.1,0.25,0.5,0.75,0.9,0.975,0.99,0.995,1))
  zs <- c(mean=zmean, mean_m_std=zmean_m_std, mean_p_std=zmean_p_std, zperc)
  idx <- sapply(zs, function(x) which.min(abs(zpte-x)))
  yp <- survfit(coxph_fit, xte[idx,])
  
 
  # save curves for test
  writeMat(file.path(base_folder, sprintf('/boot/bootstrapp_te_%d.mat', iter)), zpte=zpte1, survtime=yp$time, survprob=yp$surv, survstates=yp$states, idx=idx)
  ...
  res <- concordancefit(y, zpte, reverse=TRUE, timefix=FALSE)
